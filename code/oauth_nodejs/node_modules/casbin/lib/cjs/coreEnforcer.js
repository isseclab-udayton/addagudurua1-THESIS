...
class CoreEnforcer {
    ...
    async buildIncrementalRoleLinks(op, ptype, rules) {
        for (const rmKey of this.rmMap.keys()) {
            await this.model.buildIncrementalRoleLinks(this.rmMap.get(rmKey), op, rmKey, ptype, rules);
        }
    }
    ...
    *privateEnforce(asyncCompile = true, ...rvals) {
     ...
     */
    enforceSync(...rvals) {
        return util_1.generatorRunSync(this.privateEnforce(false, ...rvals));
    }
    /**
     ...
     */
    async enforce(...rvals) {
        return util_1.generatorRunAsync(this.privateEnforce(true, ...rvals));
    }
}
